...
function prevPeriodRange(periodo) {
  ...
  if (periodo === "semana") {
    const e = new Date(n); e.setDate(n.getDate() - 7);
  ...
  return (
    <div className="flex items-end gap-1.5 h-28">
      {dados.map((d, i) => {
        ...
        return (
          <div key={d.key ?? i} className="flex-1 flex flex-col items-center gap-1 min-w-0">
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="grid grid-cols-9 gap-1.5 sm:grid-cols-17">
        {horas.map((h) => {
          ...
          const bg =
            ...
            intensity < 0.33 ? "bg-indigo-100" :
            intensity < 0.66 ? "bg-indigo-300" : "bg-indigo-500";
          const textColor = intensity >= 0.66 ? "text-white" : "text-indigo-700";
          return (
            <div key={h} className="flex flex-col items-center gap-1">
              ...
              >
                {c > 0 && (
                  <span className={`text-[10px] font-bold ${textColor}`}>{c}</span>
                )}
              </div>
      ...
      <div className="flex items-center gap-3 flex-wrap">
        {[["bg-gray-100","Vazio"],["bg-indigo-100","Baixo"],["bg-indigo-300","Médio"],["bg-indigo-500","Alto"]].map(([c,l]) => (
...
// ─── Aba: Visão Geral ────────────────────────────────────────────
function TabGeral({ atendimentos, prevAtendimentos, periodo, produtos, setView }) {
  ...
  const grafico = Object.values(porDia).sort((a, b) => a.key.localeCompare(b.key));

  return (
    <div className="flex flex-col gap-4">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <KpiCard icon="💰" label="Faturamento" valor={BRL(fat)}
          sub={`${ok.length} concluídos`} delta={delta(fat, pFat)} />
        <KpiCard icon="✂️" label="Atendimentos" valor={totalAtend}
          sub={`${cancelados} cancelados`} delta={delta(totalAtend, prevAtendimentos.length)} />
        <KpiCard icon="💳" label="Ticket médio" valor={BRL(ticket)}
          sub="por atendimento" delta={delta(ticket, pTicket)} />
        <KpiCard icon="❌" label="Cancelamentos" valor={`${txCancel.toFixed(1)}%`}
      ...
      {grafico.length > 1 && (
        <Card>
          ...
          <BarChart
            dados={grafico}
            labelFn={(d) => { const p = d.key.split("-"); return `${p[2]}/${p[1]}`; }}
          />
        </Card>
      )}
      ...
      {estoqueBaixo.length > 0 && (
        <Card>
          <div className="flex items-center justify-between mb-3">
            ...
            <button onClick={() => setView("estoque_baixo")}
      ...
      <Card>
        ...
        {atendimentos.length === 0 ? (
          <p className="text-xs text-gray-400">Nenhum atendimento no período.</p>
        ) : (
          <div className="flex flex-col gap-2 max-h-72 overflow-y-auto">
            {[...atendimentos].reverse().slice(0, 20).map((a) => {
...
function TabServicos({ atendimentos }) {
  ...
  const lista = Object.values(mapaServico).sort((a, b) => b.receita - a.receita);
  const totalReceita = lista.reduce((s, sv) => s + sv.receita, 0);

  if (lista.length === 0)
    return (
      <Card>
        <p className="text-sm text-gray-400 text-center py-8">Nenhum serviço realizado no período.</p>
      </Card>
    );
    ...
    <div className="flex flex-col gap-4">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <Card>
          ...
          <RankingBars
            itens={lista.map((s) => ({ nome: s.nome, valor: s.count, sub: BRL(s.receita) }))}
            formatValor={(v) => `${v}x`}
            cor="bg-indigo-400"
          />
        </Card>
          ...
          <table className="w-full text-xs">
            ...
            <tbody>
              {lista.map((s) => (
                <tr key={s.nome} className="border-b border-gray-50 hover:bg-gray-50">
                  ...
                  <td className="py-2 text-right font-semibold text-gray-700">{BRL(s.receita)}</td>
                  <td className="py-2 text-right text-gray-400">
                    {totalReceita > 0 ? ((s.receita / totalReceita) * 100).toFixed(1) : 0}%
                  </td>
                </tr>
              ))}
            </tbody>
            <tfoot>
              <tr className="border-t border-gray-200">
                ...
                <td className="py-2 text-right font-bold text-gray-800">{BRL(totalReceita)}</td>
...
function TabProdutos({ comandas, produtos, setView }) {
  ...
  for (const c of comandas) {
    for (const item of [...(c.itens_bar || []), ...(c.itens_loja || [])]) {
      ...
      mapaProd[k].receita += (item.quantidade || 1) * Number(item.preco_venda || 0);
    }
  }
  const vendidos = Object.values(mapaProd).sort((a, b) => b.receita - a.receita);
  const estoqueBaixo = (produtos ?? []).filter((p) => p.quantidade <= (p.estoque_minimo ?? 1));
  ...
  return (
    <div className="flex flex-col gap-4">
      {vendidos.length > 0 ? (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <Card>
            ...
            <RankingBars
              itens={vendidos.map((p) => ({ nome: p.nome, valor: p.count, sub: BRL(p.receita) }))}
              formatValor={(v) => `${v}x`}
              cor="bg-amber-400"
            />
          </Card>
      ...
      <Card>
        ...
        {estoqueBaixo.length === 0 ? (
          <p className="text-xs text-gray-400">✅ Todos os produtos com estoque adequado.</p>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
            {estoqueBaixo.map((p) => (
              <div key={p.id}
                ...
                <div>
                  <p className="text-xs font-medium text-orange-700">{p.nome}</p>
                  <p className="text-[10px] text-orange-400">Mínimo: {p.estoque_minimo ?? 1}</p>
                </div>
                <span className="text-sm font-bold text-orange-500">{p.quantidade}</span>
              </div>
            ))}
          </div>
        )}
...
function TabHorarios({ atendimentos }) {
  ...
  if (atendimentos.length === 0)
    return (
      <Card>
        <p className="text-sm text-gray-400 text-center py-8">Nenhum atendimento no período.</p>
      </Card>
    );
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
        <KpiCard icon="🔥" label="Horário mais movimentado"
          valor={top ? `${top[0]}h` : "—"} sub={top ? `${top[1]} atend.` : ""} />
        <KpiCard icon="😴" label="Horários ociosos"
          valor={ociosos.length} sub="entre 6h–22h" />
        <KpiCard icon="📊" label="Total no período"
          valor={atendimentos.length} sub="atendimentos" />
      </div>
...
function TabClientes({ atendimentos }) {
  ...
  const ticketMedio  = totalVisitas > 0
    ? clientes.reduce((s, c) => s + c.gasto, 0) / totalVisitas : 0;

  return (
    <div className="flex flex-col gap-4">
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
        <KpiCard icon="👥" label="Clientes únicos" valor={clientes.length} sub="no período" />
        <KpiCard icon="🔁" label="Recorrentes"
          valor={recorrentes}
          sub={`${clientes.length > 0 ? ((recorrentes / clientes.length) * 100).toFixed(0) : 0}% do total`} />
        <KpiCard icon="💳" label="Ticket médio/visita" valor={BRL(ticketMedio)} sub="por atendimento" />
      </div>

      {clientes.length === 0 ? (
        <Card>
          <p className="text-sm text-gray-400 text-center py-8">Nenhum cliente no período.</p>
        </Card>
      ) : (
        <>
          <Card>
            ...
            <RankingBars
              itens={clientes.map((c) => ({ nome: c.nome, valor: c.gasto, sub: `${c.visitas}x` }))}
              formatValor={BRL}
              cor="bg-violet-400"
            />
          </Card>
                ...
                <div key={c.nome}
                  ...
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-[10px] text-gray-400">{c.visitas} visita{c.visitas !== 1 ? "s" : ""}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    <span className="text-xs font-semibold text-gray-700">{BRL(c.gasto)}</span>
                  </div>
                </div>
              ))}
            </div>
...
function TabFinanceiro({ atendimentos, comandas }) {
  ...
  const comProduto  = comandas.filter((c) => Number(c.valor_bar || 0) + Number(c.valor_loja || 0) > 0);
  const semProduto  = comandas.filter((c) => Number(c.valor_bar || 0) + Number(c.valor_loja || 0) === 0);
  const tkComProd   = comProduto.length > 0
    ? comProduto.reduce((s, c) => s + Number(c.valor_total || 0), 0) / comProduto.length : 0;
  const tkSemProd   = semProduto.length > 0
    ? semProduto.reduce((s, c) => s + Number(c.valor_total || 0), 0) / semProduto.length : 0;
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <KpiCard icon="💰" label="Receita (atend.)" valor={BRL(receitaTotal)} sub={`${ok.length} concluídos`} />
        <KpiCard icon="🧾" label="Receita (comandas)" valor={BRL(totalComandas)} sub={`${comandas.length} fechadas`} />
        <KpiCard icon="🛒" label="Ticket c/ produto" valor={BRL(tkComProd)} sub={`${comProduto.length} comandas`} />
        <KpiCard icon="✂️" label="Ticket s/ produto" valor={BRL(tkSemProd)} sub={`${semProduto.length} comandas`} />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        {receitas.length > 1 && (
          <Card>
            ...
            <RankingBars itens={receitas} formatValor={BRL} cor="bg-amber-400" />
          </Card>
        )}
      </div>
      ...
      <Card>
        ...
        <div className="flex flex-col gap-2">
          {[
            { label: "Serviços", valor: receitaSvcs || receitaTotal, icon: "✂️", cor: "bg-indigo-50 border-indigo-100" },
            { label: "Bar", valor: receitaBar, icon: "🍺", cor: "bg-amber-50 border-amber-100" },
            { label: "Loja", valor: receitaLoja, icon: "🛍️", cor: "bg-emerald-50 border-emerald-100" },
          ].map((item) => {
            ...
            return (
              <div key={item.label}
                ...
                <span className="text-sm font-bold text-gray-800">{BRL(item.valor)}</span>
              </div>
            );
          })}
        </div>
...
export default function Dashboard({ produtos, setView }) {
  ...
  const carregar = useCallback(async () => {
    ...
    try {
      ...
      const [atual, prev, cmds] = await Promise.all([
  ...
  return (
    <div className="flex flex-col gap-4 w-full">
      ...
      {loading ? (
        ...
        >
          {tab === "geral"      && <TabGeral      atendimentos={atendimentos} prevAtendimentos={prevAtendimentos} periodo={periodo} produtos={produtos} setView={setView} />}
          {tab === "servicos"   && <TabServicos   atendimentos={atendimentos} />}
          {tab === "produtos"   && <TabProdutos   comandas={comandas} produtos={produtos} setView={setView} />}
          {tab === "horarios"   && <TabHorarios   atendimentos={atendimentos} />}
          {tab === "clientes"   && <TabClientes   atendimentos={atendimentos} />}
          {tab === "financeiro" && <TabFinanceiro atendimentos={atendimentos} comandas={comandas} />}
        </motion.div>
      )}
    </div>
  );
